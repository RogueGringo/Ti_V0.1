...
| `/dw-filter` | L1 | H₀ persistent clustering and module routing |
| `/dw-ascend` | L2/L3 | Gini routing, sheaf-valued synthesis, ker(L_F) convergence |
| `/wavefront` | ALL | Full pipeline orchestrator |
| `/topological-brainstorm` | ALL | Brainstorming-as-filtration for creative work |
...
```bash
./scripts/topo.sh scan            # L0: Raw artifact ingestion
./scripts/topo.sh cluster         # L1: Persistent clustering
./scripts/topo.sh synthesize      # L2: Documentation synthesis
./scripts/topo.sh validate        # L3: Sheaf consistency check
./scripts/topo.sh serve           # Build & host the site
./scripts/topo.sh figure-it-out   # Full pipeline: L0→L1→L2→L3
```
